...
def get_sam_service(request: Request) -> SamService:
    """获取SAM服务"""
    if not hasattr(request.app.state, 'sam_service'):
        raise HTTPException(status_code=500, detail="SAM服务未初始化")
    return request.app.state.sam_service
...
async def search_by_segment(
    request_data: SamSegmentRequest,
    db: Session = Depends(get_db),
    sam_service: SamService = Depends(get_sam_service),
    model_service: ModelService = Depends(get_model_service),
    faiss_service: FaissService = Depends(get_faiss_service)
):
    """通过点击图片进行SAM抠图搜索"""
    ...
    try:
        # 调试：打印收到的请求
        api_logger.info(f"收到SAM搜索请求: x={request_data.x}, y={request_data.y}, k={request_data.k}")
        api_logger.info(f"image_base64长度: {len(request_data.image_base64) if request_data.image_base64 else 0}")
        ...
        if x <= 0 or y <= 0:
            raise HTTPException(status_code=400, detail="点击位置参数无效")
        
        # 验证K值
        k = max(1, min(k, settings.search.max_k))
        
        # 1. 解析图片
        api_logger.info(f"开始解析图片，点击位置: ({x}, {y})")
        img = base64_to_cv2(image_base64)
        img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        
        # 2. SAM 抠图
        api_logger.info("执行SAM抠图")
        cropped_img, score = sam_service.predict_mask_and_crop(img_rgb, x, y)
        
        # 3. 特征提取 (将抠出来的图转为向量)
        api_logger.info("提取抠图特征")
        
        # 将numpy数组转换为PIL Image
        pil_img = PILImage.fromarray(cropped_img)
        ...
        # 提取特征
        query_features = await model_service.extract_features(img_byte_arr)
        ...
        # 6. 转换抠图为base64返回
        cropped_img_b64 = cv2_to_base64(cv2.cvtColor(cropped_img, cv2.COLOR_RGB2BGR))
        
        # 计算搜索时间
        search_duration = time.time() - start_time
        
        api_logger.info(f"SAM搜索完成，返回{len(results)}个结果，耗时{search_duration:.3f}秒，抠图置信度: {score:.3f}")
