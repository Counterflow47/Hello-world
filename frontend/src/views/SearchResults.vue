          ...
          <div class="query-image-container">
            ...
            <div class="query-info">
              ...
              <p class="query-desc">
                ...
                <span v-else-if="queryType === 'sam'">SAM智能抠图搜索</span>
              </p>
...
onMounted(() => {
  ...
  // 调试：打印路由状态
  console.log('SearchResults页面加载，路由状态:', state)
  ...
  if (state && state.results) {
    ...
    console.log('成功加载搜索结果:', {
      results_length: results.value.length,
      queryType: queryType.value,
      searchDuration: searchDuration.value
    })
  } else if (route.query.queryType === 'sam') {
    // 从query参数加载（SAM搜索）
    try {
      results.value = JSON.parse(route.query.results || '[]')
      searchParamsResponse.value = JSON.parse(route.query.searchParams || '{}')
      queryImage.value = route.query.queryImage || ''
      queryType.value = 'sam'
      segmentScore.value = parseFloat(route.query.segmentScore || 0)
      searchDuration.value = searchParamsResponse.value?.duration || 0
      hasSearched.value = true
      
      console.log('成功加载SAM搜索结果:', {
        results_length: results.value.length,
        queryType: queryType.value,
        searchDuration: searchDuration.value,
      ...
      console.error('解析SAM搜索结果失败:', error)
      ElMessage.error('加载搜索结果失败')
      goBack()
    }
  } else {
    console.log('没有搜索结果数据，返回首页')
    // 如果没有搜索结果数据，返回首页
    goBack()
  }
})
